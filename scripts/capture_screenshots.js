/**
 * Standalone script to capture screenshots of each question type in light and dark mode.
 * 
 * Usage:
...
async function captureScreenshots() {
  ...
  try {
    ...
    for (const theme of ['light', 'dark']) {
      console.log(`🎨 Capturing screenshots for ${theme} mode...`);
      ...
      for (const question of questions) {
        ...
        const countSuffix = typeCounts[type] > 1 ? `_${typeCounts[type]}` : '';
        const filename = `question_${type}${countSuffix}${suffix}.png`;
      ...
      await page.evaluate(() => {
        ...
        if (container) {
          ...
          // Use CSS variable for background so it matches theme
      ...
      await pollContainer.screenshot({
        path: path.join(SCREENSHOT_DIR, `full_poll_container${suffix}.png`),
        omitBackground: true,
        animations: 'disabled'
      });
      console.log(`   - Saved full_poll_container${suffix}.png`);
